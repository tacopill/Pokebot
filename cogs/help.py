...
class Help(formatter.HelpFormatter):
    ...
    async def send(self, dest, content=None, embeds=None):
        ...
        help_msg = await dest.send(content=content, embed=embeds[0], delete_after=60)
        page_msg = await dest.send("There are {} help pages. Send a number to see the corresponding page. Send any other message to exit.".format(len(embeds)), delete_after=60)
        ...
        if len(cmds) == 0:
            ...
            for key in cmds[1:]:
                try:
                    ...
                    await self.send(self.destination, embeds=[self.simple_embed(title='Command "{0.name}" has no subcommands.'.format(command), color=self.color, author=self.author)])
                    return
